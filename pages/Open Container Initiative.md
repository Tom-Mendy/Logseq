...
- **Structure**: The primary components include two specifications: the **Runtime Spec** (runtime-tools) and the **Image Format Spec** (image-tools).
	- **Runtime Spec**: Defines how to run a "filesystem bundle" that is unpacked on disk, including lifecycle operations such as creating, running, stopping, and deleting [[Container]].
